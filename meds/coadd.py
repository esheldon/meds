...
class MEDSCoaddMaker(maker.MEDSMaker):
    ...
    def _write_cutouts(self, obj_range=None):
        ...
        for iobj in range(obj_range[0], obj_range[1]):
    ...
    def _write_object_cutouts(self, iobj):
        ...
        if coadd_obs is not None:
            ...
            d["start_row"][iobj, icut] = self.last_row_start + self.last_size
...
class MEDSCoadder(dict):
    ...
    def __init__(self, config, meds_obj, psfmap, seed, make_plots=False):
    ...
    def get_coadd_obs(self, iobj):
        ...
        coadder = psc.Coadder(
            obslist, jacobian=self.target_jacobian, **self["coadd"],
        )
        ...
        if self.make_plots:
            ...
            self._show_coadd(
                iobj, coadd_obs, original_coadd, original_wt, original_seg,
            )
    ...
    def _get_obslist(self, iobj):
        ...
        seglist = [
            m.interpolate_coadd_seg(iobj, i) for i in range(1, ncutout + 1)
        ]
        ...
        for i in range(ncutout):
            ...
            if False:
                ...
                T = 4.0 * fac
                gmpsf = ngmix.GMixModel([0.0, 0.0, 0.0, 0.0, T, 1.0], "gauss")
                gm0 = ngmix.GMixModel([0.0, 0.0, 0.025, 0.0, T, 1.0], "gauss")
            ...
            if self.make_plots and obs.meta["ninterp"] > 0:
                self._show_epoch(
                    iobj, icut, obs.image, seg,
                    obs.weight, obs.bmask, extra="interp"
                )
            obslist.append(obs)
            ...
            if not sides_bad:
                ...
                if bad_flags is not None:
                    sides_bad = (
                        ...
                        or np.all((bmask[-1, :] & bad_flags) != 0)
                        or np.all((bmask[:, 0] & bad_flags) != 0)
                        or np.all((bmask[:, -1] & bad_flags) != 0)
                    )
    ...
    def _interp_bad_pixels(self, obs):
        ...
        if wbad.size > 0:
            print(
                "        interpolating %d/%d masked or zero weight "
    ...
    def _show_coadd(self, iobj, obs, original_coadd,
        ...
        tab[0, 1] = images.view(
            original_coadd, nonlinear=nl, title="orig coadd", show=False
        )

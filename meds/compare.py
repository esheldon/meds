...
class Comparator(object):
    ...
    def compare_images(self, type, nrand=None):
        ...
        for icount, index in enumerate(ids):
            ...
            if type == "image" and self.png_prefix is not None:
                ...
                fname = (
                    self.png_prefix + "-imdiff-%06d-%03d.png" % (index, icut)
                )
                print("writing:", fname)
                if ok:
                    oks = "OK"
                ...
                title = (
                    "rms: %.3g mean: %.3g +/- %.3g %s" % (std, mean, err, oks)
                )
        ...
        if type == "image":
            ...
            mdiff, mdiff_std, mdiff_err = eu.stat.sigma_clip(
                means, get_err=True,
            )
    ...
    def _compare_weights(self, im1, im2):
        ...
        if not numpy.all(check):
            wbad = numpy.where(~check)
            print(
                "        %d matched worse than %g.  rms is %g"
